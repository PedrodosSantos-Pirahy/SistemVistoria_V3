...
def get_agend_v2_columns(filter_date=None):
    try:
        service = sheets_service  # já autenticado via service account
...
def receive_vistoria():
    log.info("Recebendo /vistoria")
    data = request.get_json(silent=True)

    if not data:
        return jsonify({"error": "JSON ausente ou inválido"}), 400
    ...
    "assinaturas": {
        ...
        log.info("PDF GERADO EM: %s | Existe? %s", pdf_path, os.path.exists(pdf_path))

        pdf_link = upload_file_to_drive(
            pdf_path,
            f"vistoria_{di.get('numeroOrdem','')}.pdf",
            "application/pdf"
        )
        ...
        linha = [
            ...
            di.get("chegada", ""), #Starta na Col C
            di.get("vistoria", ""),
            di.get("fim", ""),
            di.get("numeroOrdem", ""),
